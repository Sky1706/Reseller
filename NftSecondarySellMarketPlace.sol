...
contract NftSecondarySellMarketPlace is Ownable ,IERC721Receiver{
    ...
    // mapping(address => mapping (address => address))public traceSellers;
    ...
    */
    ...
    function buyNfts(uint256 _tokenId) public payable {
        ...
        if (
            NftForSale.erc20Token == address(0) && msg.value == NftForSale.price
        ) {
            payable(NftForSale.seller).transfer(msg.value);
            ...
            IERC20(NftForSale.erc20Token).transferFrom(
                msg.sender,
                NftForSale.seller,
                NftForSale.price
            );
    ...
    function cancelSell(uint256 _tokenId) public onlyAdminAndNftOwner(_tokenId) {
        ...
        emit CancelSale(_tokenId,msg.sender);
    }
    ...
    //this function is used to set the auction details
    ...
    function claimNfts(uint256 _tokenId) public payable {
        Auction storage StructAuction = mapAuction[_tokenId];
        require(
        ...
        //using safetransferfrom function for transfer the nfts
        payable(StructAuction.auctioner).transfer(fundsByBidders[_tokenId][msg.sender]);
    ...
    function withdraw(uint256 _tokenId) public {
        Auction storage StructAuction = mapAuction[_tokenId];
        if (StructAuction.auctionCancel == true) {
            uint256 bidAmount = fundsByBidders[_tokenId][msg.sender];
            address bidder = StructAuction.currentBidders;

            if (StructAuction.erc20Token == address(0)) {
                payable(bidder).transfer(bidAmount);
            } else {
                IERC20(StructAuction.erc20Token).transfer(bidder, bidAmount);
            }
        } else {
            uint256 bidAmount = fundsByBidders[_tokenId][msg.sender];
            address bidder = StructAuction.currentBidders;
    ...
    function onERC721Received(address _from,address _to,uint256 _tokenId,bytes memory _data)public virtual override returns(bytes4){
        ...
        (uint256 tokenId,uint256 price,uint256 basePrice,address erc20Token,uint256 saleType,address sellerAndAuctioner) = abi.decode(_data,(uint256,uint256,uint256,address,uint256,address));
